    ...
    def extract(self, filepath, existing):
        ...
        for stmt in ofx.statements:
            ...
            for invpos in stmt.invposlist:
                print("InvPos: {}\n".format(invpos))
                #if type(invpos) is POSDEBT:
                    #pamt = Amount(Decimal(1), self.bond_prefix + txn.secid.uniqueid)
                    #entries.append(Price(new_metadata(filepath, 0), date, security.ticker, amount))
                if type(invpos) is POSSTOCK:
                    ticker = tickers[invpos.secid.uniqueid]
                    bact = self.get_account(ticker)
                    bamt = Amount(invpos.units, ticker)
                    entries.append(Balance(new_metadata(filepath, 0), asofdate, bact, bamt, None, None))
